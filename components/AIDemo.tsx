    ...
    <section className="border-b border-slate-200 bg-slate-50 py-16 sm:py-24 lg:py-28">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-3xl lg:max-w-none">
          <h2 className="text-2xl font-extrabold leading-[1.15] tracking-tight text-slate-900 sm:text-4xl md:text-5xl sm:whitespace-pre-line">
            {t.aiDemo.title}
          </h2>

          <Card className="mt-8 mx-auto max-w-2xl rounded-2xl border border-slate-200 bg-white p-0 shadow-sm sm:mt-12">
            <CardContent className="flex flex-col gap-4 rounded-2xl p-4 sm:gap-6 sm:p-6">
              <div className="flex items-center gap-2.5 border-b border-slate-200 pb-3 sm:gap-3 sm:pb-4">
                ...
                <div>
                  <h3 className="text-sm font-semibold text-slate-900 sm:text-base">{t.aiDemo.assistantName}</h3>
                    ...
                    >
                      <div
                        className={`flex h-7 w-7 flex-shrink-0 items-center justify-center rounded-full sm:h-8 sm:w-8 ${
                          msg.type === "user" ? "bg-slate-900 text-white" : "bg-emerald-700 text-white"
                        }`}
                      >
                        {msg.type === "user" ? (
