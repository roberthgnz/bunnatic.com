...
function getServicesFromSource(
  ...
  const fromTypesArray = Array.isArray(source.types)
    ? source.types
        ...
        .filter(
          (typeValue) => typeValue.length > 0 && !IGNORE_TYPES.has(typeValue)
        )
  ...
  const fromCapabilities = [
    ...
    source.wheelchair_accessible_entrance
...
function getHoursFromSource(
  ...
  for (const period of periods) {
    ...
    const nextOpen =
      current.open_time && current.open_time < openTime
        ? current.open_time
        : openTime
    const nextClose =
...
export function mapStripePriceIdToGenerationPlan(
...
export function buildBusinessSourcePreview({
  ...
  const profile: SourceProfilePreview = {
    ...
    googlePlaceId:
      sourceType === 'google' ? pickFirstString(source.place_id) : '',
