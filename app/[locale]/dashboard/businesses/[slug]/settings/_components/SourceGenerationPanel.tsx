        ...
        {sourceType === "google" ? (
          <div className="space-y-3">
            ...
            {googleResults.length > 0 && (
              <div className="rounded-xl border border-slate-200 bg-slate-50/80 p-2 space-y-1.5 animate-in fade-in slide-in-from-top-2 duration-200">
                {googleResults.map((result) => (
                  <button
                    key={result.place_id}
                    type="button"
                    className="flex w-full items-start gap-3 rounded-lg border border-slate-200 bg-white px-4 py-3 text-left transition-all hover:border-blue-200 hover:bg-blue-50/50 disabled:opacity-50"
            ...
            {googleResults.length === 0 && !isSearching && (
              <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-slate-200 bg-slate-50/50 py-8 text-center gap-2">
                <div className="flex h-10 w-10 items-center justify-center rounded-full bg-slate-100">
                  <Search className="h-5 w-5 text-slate-400" />
                </div>
                <p className="text-sm text-slate-500 max-w-xs">{t.selectBusiness}</p>
              </div>
            )}
          </div>
        ) : (
