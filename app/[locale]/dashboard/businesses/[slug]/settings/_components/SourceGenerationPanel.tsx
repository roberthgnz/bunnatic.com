...
export default function SourceGenerationPanel({
    ...
    () =>
      ({
        es: {
          ...
          subtitle: "Usa los datos reales de tu negocio desde Google My Business o desde tu web para regenerar información.",
          ...
          searchButton: "Buscar",
          searching: "Buscando...",
          selectBusiness: "Busca tu negocio en Google para generar un preview automático.",
        ...
        ca: {
          ...
          subtitle: "Fes servir les dades reals del teu negoci des de Google My Business o des del teu web per regenerar informació.",
          ...
          searchButton: "Cercar",
          searching: "Cercant...",
          selectBusiness: "Cerca el teu negoci a Google per generar una vista prèvia automàtica.",
  ...
  const buildPreviewFromPayload = async (nextSourceType: SourceType, sourcePayload: unknown) => {
    ...
    try {
      const result = await buildBusinessSourcePreviewAction({ sourceType: nextSourceType, sourcePayload });
      ...
      trackFunnelEvent("preview_generated", { source_type: nextSourceType, locale: safeLocale });
    } catch (error) {
      console.error(error);
  ...
  const handleSelectGoogleResult = async (result: PlaceSearchResult) => {
    setIsBuildingPreview(true);
    try {
      const response = await fetch(`/api/places/details?place_id=${encodeURIComponent(result.place_id)}&lang=${safeLocale}`);
  ...
  const handleCrawlUrl = async (values: UrlCrawlValues) => {
    ...
    try {
      ...
      while (Date.now() - startedAt < timeoutMs) {
        ...
        if (!statusResponse.ok) throw new Error(statusData.error || t.errorCrawl);
  ...
  const handleApply = async () => {
    if (!preview) return;
    if (isLimitReached) {
      trackFunnelEvent("limit_blocked", { source_type: sourceType, locale: safeLocale });
    ...
    setIsApplying(true);
    trackFunnelEvent("apply_submitted", { source_type: sourceType, locale: safeLocale, blocks_count: blocks.length });
    ...
    try {
      ...
      if ("error" in result && result.error) {
        if (result.limitBlocked) trackFunnelEvent("limit_blocked", { source_type: sourceType, locale: safeLocale });
      ...
      trackFunnelEvent("apply_success", { source_type: sourceType, locale: safeLocale, blocks_count: blocks.length });
      if (result.entitlement) setEntitlement(result.entitlement);
  ...
  const blockDefs: { key: SourceBlock; icon: React.ReactNode; label: string; desc: string }[] = [
    { key: "profile", icon: <User className="h-4 w-4" />, label: t.blockProfile, desc: t.blockProfileDesc },
    { key: "services", icon: <Briefcase className="h-4 w-4" />, label: t.blockServices, desc: t.blockServicesDesc },
    { key: "hours", icon: <Clock className="h-4 w-4" />, label: t.blockHours, desc: t.blockHoursDesc },
  ];
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden">
      {/* Header */}
      <div className="bg-gradient-to-r from-slate-900 to-slate-800 px-6 py-5 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400 max-w-sm leading-relaxed">{t.subtitle}</p>
          </div>
        </div>
        {/* Quota badge */}
        <div className="shrink-0">
          {entitlement?.isLimited ? (
            <div className={`inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-xs font-medium ring-1 ${isLimitReached ? "bg-red-900/40 text-red-300 ring-red-700/40" : "bg-blue-900/30 text-blue-300 ring-blue-700/30"}`}>
      ...
      <div className="p-6 space-y-6">
        ...
        <div className="space-y-2">
          <p className="text-xs font-semibold uppercase tracking-widest text-slate-400">{t.sourceLabel}</p>
        ...
        {sourceType === "google" ? (
          <div className="space-y-3">
              ...
              <div className="relative flex-1 space-y-1.5">
                <div className="relative">
                  ...
                  <Input
                    ...
                    className="pl-9 h-10 border-slate-200 bg-white focus-visible:ring-blue-500/30 text-sm"
                  />
                </div>
                {googleSearchForm.formState.errors.query && (
                  <p className="flex items-center gap-1 text-xs font-medium text-destructive">
                    <svg className="h-3 w-3 shrink-0" viewBox="0 0 16 16" fill="currentColor" aria-hidden><path fillRule="evenodd" d="M8 1a7 7 0 1 0 0 14A7 7 0 0 0 8 1zm.75 3.75a.75.75 0 0 0-1.5 0v3.5a.75.75 0 0 0 1.5 0v-3.5zM8 11a.875.875 0 1 0 0-1.75A.875.875 0 0 0 8 11z" clipRule="evenodd" /></svg>
                    {t.errorSearch}
                  </p>
                )}
              </div>
            ...
            {googleResults.length > 0 && (
              <div className="rounded-xl border border-slate-200 bg-slate-50/80 p-2 space-y-1 animate-in fade-in slide-in-from-top-2 duration-200">
                {googleResults.map((result) => (
                  ...
                  >
                    <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-blue-50 ring-1 ring-blue-100 group-hover:bg-blue-100 transition-colors">
                      <MapPin className="h-4 w-4 text-blue-600" />
                    </div>
                    <div className="min-w-0 flex-1">
                      <p className="text-sm font-semibold text-slate-900">{result.name}</p>
                      <p className="text-xs text-slate-500 truncate">{result.formatted_address}</p>
                    </div>
                    <Search className="h-3.5 w-3.5 shrink-0 text-slate-300 group-hover:text-blue-400 transition-colors" />
                  </button>
                ))}
              </div>
            )}
            ...
            {googleResults.length === 0 && !isSearching && (
              <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-slate-200 bg-slate-50/50 py-10 text-center gap-3">
                <div className="flex h-11 w-11 items-center justify-center rounded-full bg-white ring-1 ring-slate-200 shadow-sm">
                  <Search className="h-5 w-5 text-slate-400" />
                </div>
                <div>
                  <p className="text-sm font-medium text-slate-600">{t.sourceGoogle}</p>
                  <p className="mt-0.5 text-xs text-slate-400 max-w-xs">{t.selectBusiness}</p>
                </div>
              </div>
            )}
          </div>
        ...
        {(isBuildingPreview || isCrawling) && (
          <div className="flex items-center justify-center gap-3 rounded-xl border border-blue-100 bg-blue-50 py-5 animate-in fade-in duration-300">
            <Loader2 className="h-5 w-5 animate-spin text-blue-600" />
            <span className="text-sm font-medium text-blue-700">{t.generatingPreview}</span>
          </div>
        )}
          ...
          <>
            {preview ? (
              <div className="space-y-5 animate-in fade-in slide-in-from-bottom-2 duration-300">
                {/* Block selector */}
                <div className="space-y-2">
                  <p className="text-xs font-semibold uppercase tracking-widest text-slate-400">{t.blocksTitle}</p>
                  <div className="grid grid-cols-1 gap-2 sm:grid-cols-3">
                    {blockDefs.map(({ key, icon, label, desc }) => {
                      ...
                      return (
                        ...
                        >
                          <div className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-lg ${isSelected && !disabled ? "bg-blue-100 text-blue-600" : "bg-slate-100 text-slate-500"}`}>
                            {icon}
                          </div>
                          <div className="min-w-0 flex-1">
                            <p className={`text-sm font-semibold ${isSelected && !disabled ? "text-blue-800" : "text-slate-800"}`}>{label}</p>
                            <p className="text-xs text-slate-500 leading-relaxed">{desc}</p>
                          </div>
                          <div className={`mt-0.5 shrink-0 ${isSelected && !disabled ? "text-blue-500" : "text-slate-300"}`}>
                            {isSelected && !disabled ? <CheckSquare className="h-4 w-4" /> : <Square className="h-4 w-4" />}
                          </div>
                        </button>
                      );
                    })}
                ...
                <div className="grid gap-4 md:grid-cols-2">
                  ...
                  <div className="rounded-xl border border-slate-200 bg-slate-50 p-4 space-y-3">
                    <div className="flex items-center gap-2">
                      ...
                      <p className="text-xs font-semibold uppercase tracking-widest text-slate-400">{t.blockProfile}</p>
                    </div>
                    <div className="space-y-2">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="rounded-xl border border-slate-200 bg-slate-50 p-4">
                      <div className="flex items-center gap-2 mb-3">
                        ...
                        <p className="text-xs font-semibold uppercase tracking-widest text-slate-400">{t.blockServices}</p>
                      </div>
                      {preview.services.length > 0 ? (
                        <ul className="space-y-1">
                          {preview.services.map((service) => (
                            <li key={service.name} className="flex items-center gap-2 text-xs text-slate-700">
                        ...
                        <p className="text-xs text-slate-400 italic">{t.noServices}</p>
                      )}
                    </div>
                    ...
                    <div className="rounded-xl border border-slate-200 bg-slate-50 p-4">
                      <div className="flex items-center gap-2 mb-3">
                        ...
                        <p className="text-xs font-semibold uppercase tracking-widest text-slate-400">{t.blockHours}</p>
                      </div>
                      {preview.hours.length > 0 ? (
                        <ul className="space-y-1">
                          {preview.hours.map((hour) => (
                            ...
                            >
                              <span className="font-medium text-slate-600">{hour.day_of_week}</span>
                              <span className={hour.is_closed ? "text-slate-400 italic" : "text-slate-800"}>
                        ...
                        <p className="text-xs text-slate-400 italic">{t.noHours}</p>
                      )}
                    </div>
                  </div>
                </div>
              ...
              <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-slate-200 bg-slate-50/50 py-12 text-center gap-3">
