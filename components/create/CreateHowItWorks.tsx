...
import { CheckCircle2, Globe, Loader2, MapPin, MessageCircle, Search } from "lucide-react";
...
export default function CreateHowItWorks({ copy, query }: CreateHowItWorksProps) {
  const steps = [
    {
      number: "1",
        ...
        <motion.div className="space-y-3" variants={cardVariants}>
          ...
          <motion.div className="overflow-hidden" variants={revealResult}>
            <div className="flex items-center gap-2 px-1 text-xs font-medium text-gray-900">
              ...
              <span className="truncate">{query.trim() || "Pizzería Napoli, Madrid"}</span>
            </div>
          </motion.div>
        </motion.div>
      ),
      ...
      mockup: (
        <motion.div className="space-y-3.5" variants={cardVariants}>
          ...
          <ul className="space-y-2.5 text-xs text-gray-600">
            ...
            <motion.li className="flex items-center gap-2 overflow-hidden text-gray-400" variants={loadingReveal}>
              <motion.div
          ...
          <div className="h-1.5 w-full overflow-hidden rounded-full bg-gray-200">
            <motion.div variants={progressPulse} className="h-full w-full bg-violet-500" />
          </div>
        </motion.div>
      ),
    },
  ...
  return (
    <section className="mt-8 rounded-3xl bg-slate-50 p-4 sm:p-6 ring-1 ring-slate-200/80">
      <div className="mx-auto max-w-xl text-center">
        <h4 className="text-xl font-extrabold tracking-tight text-gray-900 sm:text-2xl">{copy.howToTitle}</h4>
        <p className="mt-2 text-sm text-gray-600 sm:text-base">{copy.howToSubtitle}</p>
      </div>
              ...
              <Card className="rounded-3xl bg-white shadow-sm ring-1 ring-gray-200 transition-all hover:shadow-md">
                <CardContent className="p-5 sm:p-6">
                  <div className="flex items-center gap-3 text-base font-bold text-gray-900">
                    <span className={`flex h-9 w-9 items-center justify-center rounded-xl text-sm text-white ${step.color}`}>
                      {step.number}
                    </span>
                    {step.title}
                  </div>
