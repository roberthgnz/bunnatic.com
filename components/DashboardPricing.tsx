...
const STRIPE_PRICES = {
  ...
  "tier-agency": {
    ...
    yearly: process.env.NEXT_PUBLIC_STRIPE_PRICE_AGENCY_YEARLY || "price_agency_yearly",
  },
  "tier-scale": {
    monthly: process.env.NEXT_PUBLIC_STRIPE_PRICE_SCALE_MONTHLY || "price_scale_monthly",
    yearly: process.env.NEXT_PUBLIC_STRIPE_PRICE_SCALE_YEARLY || "price_scale_yearly",
  },
};
...
export default function DashboardPricing() {
  ...
  const handleSubscribe = async (tierId: string) => {
    try {
      ...
      const priceId = STRIPE_PRICES[tierId as keyof typeof STRIPE_PRICES]?.[isAnnual ? 'yearly' : 'monthly'];
    ...
    <div className="py-8">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-3xl text-center">
          ...
          <p className="mt-3 text-base text-slate-600 sm:mt-4 sm:text-lg">{t.pricing.subtitle}</p>
                ...
                >
                  <CardContent className="flex h-full flex-1 flex-col p-5 sm:p-8">
                    <div>
                      ...
                      <p className="mt-2 text-base text-slate-600 sm:mt-3 sm:text-lg">{tier.description}</p>
                    </div>
                  ...
                  <ul className="mt-8 space-y-4 sm:mt-10 sm:space-y-5">
                    {tier.features.map((feature) => (
                      <li key={feature.title} className="flex items-start gap-3">
                        ...
                        <div className="space-y-1">
                          <p className="text-lg font-extrabold text-slate-900 sm:text-xl">{feature.title}</p>
