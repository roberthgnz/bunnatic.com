...
import { Loader2, Search, MapPin, Star, CheckCircle2, ArrowLeft, Globe } from "lucide-react";
  ...
  const handleCrawl = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      while (Date.now() - startedAt < timeoutMs) {
        ...
        if (!statusResponse.ok) throw new Error(statusData.error || t.detailsError);
          ...
          {sourceType === "google" ? (
            <>
              <form onSubmit={handleSearch} className="flex flex-col gap-3 sm:flex-row">
                <Input
                  ...
                  required
                />
                <Button type="submit" disabled={isSearching || isCrawling} className="sm:w-36">
                  {isSearching ? (
                    <>
            ...
            <form onSubmit={handleCrawl} className="flex flex-col gap-3 sm:flex-row">
              <Input
                ...
                required
              />
              <Button type="submit" disabled={isSearching || isCrawling} className="sm:w-40">
                {isCrawling ? (
                  <>
                    <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                    {t.crawling}
