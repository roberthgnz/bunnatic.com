...
export async function POST(request: Request) {
  ...
  if (!body?.placeData || typeof body.placeData !== "object") {
    return NextResponse.json({ error: "placeData is required" }, { status: 400 });
  }
...
export async function GET(request: Request) {
  ...
  if (!record) {
    return NextResponse.json({ error: "Temporary generation not found" }, { status: 404 });
  }
  ...
  const mapped = {
    ...
    description:
      typeof placeData.editorial_summary?.overview === "string"
        ...
        : "",
    address: typeof placeData.formatted_address === "string" ? placeData.formatted_address : "",
    phone:
    ...
    googlePlaceId: typeof placeData.place_id === "string" ? placeData.place_id : "",
    placeData,
