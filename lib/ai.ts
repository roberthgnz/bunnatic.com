export async function generateBusinessContent(name: string, category: string) {
  ...
  const isRestaurant = category.toLowerCase().includes('restaurante') || category.toLowerCase().includes('bar') || category.toLowerCase().includes('cafetería');
  const isSalon = category.toLowerCase().includes('peluquería') || category.toLowerCase().includes('estética') || category.toLowerCase().includes('belleza');
  ...
  if (isRestaurant) {
    ...
    services.push(
      { name: 'Menú del Día', description: 'Primer plato, segundo, postre y bebida.', price: 12.50, duration: 60 },
      { name: 'Cena Degustación', description: 'Selección de nuestros mejores platos.', price: 35.00, duration: 90 },
      { name: 'Desayuno Completo', description: 'Café, zumo y tostada o bollería.', price: 4.50, duration: 30 }
    )
  } else if (isSalon) {
    ...
    services.push(
      { name: 'Corte de Pelo', description: 'Asesoramiento, lavado y corte.', price: 20.00, duration: 45 },
      { name: 'Tinte Completo', description: 'Coloración profesional y cuidado capilar.', price: 45.00, duration: 90 },
      { name: 'Peinado Evento', description: 'Recogidos y peinados para ocasiones especiales.', price: 30.00, duration: 60 }
    )
  }
