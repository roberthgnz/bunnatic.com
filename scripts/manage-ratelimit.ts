...
if (!url || !token) {
  console.error('❌ Error: UPSTASH_REDIS_REST_URL y UPSTASH_REDIS_REST_TOKEN deben estar configurados');
  process.exit(1);
}
...
async function checkRateLimit(ip: string) {
  ...
  console.log(`   Reset: ${resetDate.toLocaleString()} (en ${hoursUntilReset}h)`);
