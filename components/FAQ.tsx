        ...
        <div className="mx-auto max-w-4xl">
          ...
          <Accordion type="single" collapsible className="mt-6 space-y-3 sm:mt-10 sm:space-y-4">
            {t.faq.questions.map((faq, index) => (
              ...
              >
                <AccordionTrigger className="flex w-full items-center justify-between py-3 text-slate-900 hover:no-underline sm:py-4 **:data-[slot=accordion-trigger-icon]:ml-auto **:data-[slot=accordion-trigger-icon]:size-4 sm:**:data-[slot=accordion-trigger-icon]:size-5 **:data-[slot=accordion-trigger-icon]:text-slate-500">
                  <span className="text-base font-semibold leading-6 sm:text-lg sm:leading-7">{faq.q}</span>
                </AccordionTrigger>
                <AccordionContent>
                  <p className="pb-4 text-sm leading-6 text-slate-600 sm:pb-6 sm:text-base sm:leading-7">{faq.a}</p>
                </AccordionContent>
              </AccordionItem>
            ))}
          </Accordion>
